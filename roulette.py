...
def game(username, bet, choice):
    ...
    if balance < bet:
        return f"❌ {name}, you don't have enough balance to place this bet! Your balance: {balance}."
    ...
    if choice in valid_colors:
        if choice == winning_color:
            ...
            return f"🎉 {name} bet on {choice} and won! 🎊 The ball landed on {winning_color}. New balance: {new_balance}"
        else:
            ...
            return f"💀 {name} bet on {choice} but lost. The ball landed on {winning_color}. New balance: {new_balance}"
